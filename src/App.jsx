...
export default function App() {
  ...
  function addTask(title) {
    setTasks(prev => [...prev, { id: crypto.randomUUID(), title: title.trim(), done: false, createdAt: Date.now() }])
  }
  function toggle(id) {
  ...
  return (
    <div className="shell">
      ...
      <main className="panel">
        <h1>HiQode Tasks</h1>
        <p className="muted">Double‑click a task to edit. Click the checkbox to mark done.</p>
        ...
        <ul className="list">
          {filtered.map(t => (
            <TaskItem key={t.id} task={t} onToggle={() => toggle(t.id)} onEdit={(v) => edit(t.id, v)} onRemove={() => remove(t.id)} />
          ))}
        </ul>
